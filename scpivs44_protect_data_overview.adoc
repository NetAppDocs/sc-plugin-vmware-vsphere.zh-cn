...
---
= SnapCenter Plug-in for VMware vSphere 的数据保护工作流程
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 使用 NetApp Backup and Recovery 增强 VMware 保护，它提供 3-2-1 备份、勒索软件保护、自动备份管理、改进的可扩展性、降低的成本以及对多个 vCenter 实例的集中管理。您可以将 SCV 策略、快照和计划迁移到 NetApp Backup and Recovery，并从单个控制台管理所有虚拟机保护。有关详细信息，请参阅 link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery 文档"]。
