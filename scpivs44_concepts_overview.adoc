...
---
= 了解 SnapCenter Plug-in for VMware vSphere
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: 通过 NetApp Backup and Recovery 增强 VMware 保护，该解决方案提供 3-2-1 备份、勒索软件防护、自动化备份管理、可扩展性提升、成本降低，以及通过单一界面集中管理多个 vCenter 实例中的数据存储和虚拟机的备份。您可以将 SCV 策略、快照和计划迁移到 NetApp Backup and Recovery，并通过单一控制台管理所有虚拟机保护。如需更多信息，请参阅 link:https://docs.netapp.com/us-en/data-services-backup-recovery/["NetApp Backup and Recovery 文档"]。

====
SnapCenter Plug-in for VMware vSphere 是一种独立的虚拟设备（Open Virtual Appliance 格式），可为 VM 和数据存储提供数据保护服务，并支持 SnapCenter 基于应用程序的插件的数据保护服务。本文档介绍如何部署和使用 SnapCenter Plug-in for VMware vSphere，并包含快速入门信息。SnapCenter Plug-in for VMware vSphere 部署为基于 Linux 的虚拟设备。
