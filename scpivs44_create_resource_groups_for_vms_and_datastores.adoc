...
* 对于 MetroCluster 环境中的 FlexGroup 卷，如果您使用的是 ONTAP 9.8 或 9.9，请在切换或切回后，备份资源组之前，重启 SnapCenter Plug-in for VMware vSphere 服务并重新同步 SnapMirror 关系。在 ONTAP 9.8 中，切回后备份可能会挂起；此问题在 ONTAP 9.9 中已解决。
